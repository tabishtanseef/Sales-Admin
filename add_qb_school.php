...
if (isset($_POST['add_school'])) {
	...
	$g = "SELECT school_name FROM qb_school_list WHERE school_name ='$name' and school_address='$address' ";
	...
	if (!$error) {
		...
		$g = "SELECT * FROM qb_school_list WHERE school_name ='$name' and school_address='$address' and school_city='$city'";
		$r = mysqli_query($conn,$g);
		$ch = mysqli_fetch_array($r);
		if (strlen($ch['id']) != 0)
		{
			...
			if(mysqli_query($conn, "INSERT INTO qb_school_list(user_id, user_name, school_name, school_board, school_strength, school_address, school_city, school_state) VALUES('". $u_id ."','". $u_name ."','". $name ."', '" . $board . "', '". $strength ."', '". $address ."', '". $city ."', '". $state ."')")) {
