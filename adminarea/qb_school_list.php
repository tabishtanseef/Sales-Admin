...
if (isset($_POST['go'])) {
	...
	if(!$error){
		header ("location:qb_school_list.php?salesman=$salesman&state=$state&city=$city");	
	}
...
while($row_attendance=mysqli_fetch_array($run_attendance))
	{
		...
		<td><img src='img/del.png' onclick='delete_qb_school($school_id)' style='width:28px; margin:auto;'></td>
		<td>$n</td>
...
<body>
			...
			<div class="row sub" style="margin-top:2%;">
				<div class="col-sm-12 horizontal-scroll">
					<table id="school_list" class="table table-hover table-bordered table-striped table-responsive w-100 d-block d-md-table" style="width:100%;">
						...
						$get_attendance="select * from qb_school_list where user_name ='$salesman' AND school_state = '$state' and school_city='$city' order by school_name";
						$run_attendance= mysqli_query($conn, $get_attendance);
						...
						$get_attendance="select * from qb_school_list where user_name ='$salesman' AND school_state = '$state' order by school_name";
						$run_attendance= mysqli_query($conn, $get_attendance);
						...
						$get_attendance="select * from qb_school_list where school_state = '$state' AND school_city='$city' order by school_name";
						$run_attendance= mysqli_query($conn, $get_attendance);
						...
						$get_attendance="select * from qb_school_list where user_id = '$salesman_id' order by school_name";
						$run_attendance= mysqli_query($conn, $get_attendance);
						...
						$get_attendance="select * from qb_school_list where school_state = '$state' order by school_name";
						$run_attendance= mysqli_query($conn, $get_attendance);
						 
						school($run_attendance);
						}
						else{
						$get_attendance="select * from qb_school_list order by school_name";
						$run_attendance= mysqli_query($conn, $get_attendance);
    ...
    <script type="text/javascript">
		...
		function delete_qb_school(school_id){
			$("#"+school_id).addClass('hidden');
			$.ajax({
			url: 'delete_qb_school.php',
			type: 'GET',
